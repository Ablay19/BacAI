...
const solveSchema = z.object({
  ...
  subject: z.enum(['mathematics', 'arabic', 'french', 'english', 'sciences', 'islamic_studies']).optional(),
  level: z.enum(['secondary_basic', 'secondary_lycee', 'university']).optional(),
...
solveRouter.post('/', zValidator('json', solveSchema), async (c) => {
  try {
    ...
    const detectedLanguage = validatedData.language || detectLanguage(validatedData.exercise)
    
    // Auto-detect subject if not provided
    const detectedSubject = validatedData.subject || extractSubject(validatedData.exercise, detectedLanguage)
